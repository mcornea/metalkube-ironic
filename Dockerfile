...
RUN cp /etc/ironic/ironic.conf /etc/ironic/ironic.conf_orig && \
    ...
    crudini --set /etc/ironic/ironic.conf DEFAULT my_ip 172.22.0.2 && \
    ...
    crudini --set /etc/ironic/ironic.conf conductor api_url http://172.22.0.2:6385 && \
    crudini --set /etc/ironic/ironic.conf deploy http_url http://172.22.0.2:5280 && \
    ...
    crudini --set /etc/ironic/ironic.conf inspector endpoint_override http://172.22.0.2:5050 && \
